...
public class DCFProblem implements Parser,Saver, DistributedConsequenceFindingProblem<SolProblem>{
	...
	public void generateFrom(SolProblem source, String distribProgASP, int distribType, int nbAg, double minProportion,
		...
		for (IndepLiteral hasClause:models.get(0).getLiterals()){
	...
	public Pair<Integer,Integer> convertProportion(SolProblem source, int nbAg, 
		...
		int nbClauses=source.getNbClauses();
	...
	public void interpretHasClause(IndepLiteral hasClauseLit, SolProblem source,List<String>agentNames){
		if (!hasClauseLit.getPredicate().equals("hasClause"))
			return;
		String agName=hasClauseLit.getArguments().get(0).trim();
		int agent=agentNames.indexOf(agName);
		if (agent<0){
			agent=agentNames.size();
			agentNames.add(agName);
		}
		int ind=Integer.parseInt(hasClauseLit.getArguments().get(1).trim());
		localProblems.get(agent).addClause(source.getClause(ind), source.isTopClause(ind));
	}
	...
	public File makeDistribProgram(SolProblem source, String distribProgASP, int distribType,
		...
		case ASPCOD_DLV_NAIVE:
			...
			// set constants
			setConstants(fileOut, nbMinAg, maxComm, maxCl, minCl, source.getNbClauses());
			// combine with asp distributor program			
		case ASPCOD_CLASP_CUT:
			// get source asp form
			source.generateConnectionClaspFacts(fileOut);
			// set constants
			setConstants(fileOut, maxAg, nbMinAg, maxComm, maxCl, minCl, source.getNbClauses());
			// combine with asp distributor program
...
//		p.println();
		...
		for (int ag=0;ag<localProblems.size();ag++){
			...
			for (int i=0;i<pb.getNbClauses();i++){
				if (pb.isTopClause(i))
					clause=pb.getClause(i).toSolFileLine("top_clause");
				else
					clause=pb.getClause(i).toSolFileLine("axiom");
	...
	//	if (pb==null) throw new IOException("Declare an agent before using it - agent "+agent+"in cnf "+line);
		pb.addClause(clause,role.equals("top_clause"));
	}
