...
public class TokenProtocol extends BaseMainProtocol implements CFMessageTypes{
	...
	protected void receiveCl(TokenMessageArgument arg){
		...
		Collection<IndepClause> tempCsq=new ArrayList<IndepClause>();
		//Initialisation
		if (firstRun) {
			firstRun=false;
			allFlagged=true; // each agent should run at least once in the given order
			newCl.addAll(ag.getAllTopClauses());
			newCsq.put(FORWARD, new ArrayList<IndepClause>());
			newCsq.put(BACKWARD, new ArrayList<IndepClause>());
		}
		...
		if (!newCsq.get(dir).isEmpty() || !arg.getCurrentFlags().isEmpty() || (allFlagged)){
			...
			Collection<IndepClause> sentCl=new ArrayList<IndepClause>();
	...
	protected HashMap<Integer,Collection<IndepClause>> newCsq=new HashMap<Integer,Collection<IndepClause>>();
