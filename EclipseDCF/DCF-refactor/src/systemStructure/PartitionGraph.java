...
public class PartitionGraph<ConsFindingAgent extends TheoryAgent> {
	...
	public IndepPField getOutputLanguage(int i){
		...
		return new IndepPField(lits);
	}
	
	
	public IndepPField getInputLanguage(int i){
		List<IndepLiteral> lits=new ArrayList<IndepLiteral>();
	...
	public IndepPField getCommLanguage(int i, int j){
		List<IndepLiteral> lits=new ArrayList<IndepLiteral>();
	 ...
	 *
	 */
	public void computeLocalCommLanguages() {
		Pair<Integer,Integer> set1,set2;
	 ...
	 */
	private HashSet<IndepLiteral> searchCommonLiterals(TheoryAgent a1, TheoryAgent a2) {
		Collection<IndepLiteral> listeC1 = new ArrayList<IndepLiteral>();
		Collection<IndepLiteral> listeC2 = new ArrayList<IndepLiteral>();
		
		if (polarizedCommLang){
			listeC1 = a1.getVocabulary();
			listeC2 = a2.getNegatedVocabulary();
		}
		else {
			listeC1 = a1.getFullVocabulary();
			listeC2 = a2.getFullVocabulary();
		}
		...
		for (int i=0;i<nbAgents;i++){
			if (i!=root) for (int j=i+1;j<nbAgents;j++){
				if (j!=root){
					...
					Collection<IndepLiteral> labels = ConnectGraphLabels.get(edgeToRemove);
	...
	public void removeEdge(Pair<Integer,Integer> edge, List<List<Integer>> alternatePaths){
		...
		Collection <IndepLiteral> addLabel=new ArrayList<IndepLiteral>();
	...
	private void addLiteralToPath(IndepLiteral lit, List<Pair<Integer,Integer>> path){
	...
	private int evaluateAddLitToPath(IndepLiteral lit, List<Pair<Integer,Integer>> path){
		int n=0;
		Collection <IndepLiteral> tempTarget=new ArrayList<IndepLiteral>();
		for (Pair<Integer,Integer> e:path){
			//avoiding redundancy (counting on symmetry of labels not to do it twice)
